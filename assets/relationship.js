...
/***/ function(module, exports, __webpack_require__) {
	...
	var caches = {
	    //节点数据
	    nodes: [{ name: 'James', type: _PERSON_TYPE }, { name: 'Irvin', type: _PERSON_TYPE }, { name: 'Love', type: _PERSON_TYPE }, { name: '队友', type: _RELATION_TYPE }, { name: '好友', type: _RELATION_TYPE }, { name: '亲戚', type: _RELATION_TYPE }],
	    //连线数据
	    edges: [//从人物指向关系
	...
	var relationshipMain = {
	    ...
	    initEvent: function initEvent() {
	        ...
	        caches.svgEle.on('mouseup', function () {
	            ...
	            if (triggerBrushPrepare && new Date().getTime() - beginTime < 200) {
	                ...
	                console.log(d3.event.target, d3.event.target.nodeValue);
	                var target = d3.event.target;
	                document.getElementsByTagName('button')[0].click();
	                // d3.select(target).click();
	            }
	        });
	    ...
	    createSvgEle: function createSvgEle() {
	        caches.svgEle = d3.select('body').insert('svg', 'script').attr('width', caches.svgWidth).attr('height', caches.svgHeight).style({ border: '1px solid black' });
	    },
	    ...
	    renderCircles: function renderCircles() {
	        ...
	        }).style({ stroke: 'black', 'stroke-width': 2 }).call(caches.forceLayout.drag); //允许拖动
	    },
	    ...
	    /**tick监听器:当节点运动的时候调用*/
	    tickEventListener: function tickEventListener() {
	    ...
	    /**点击单个节点事件监听*/
	    clickSingleCircleEventListener: function clickSingleCircleEventListener() {
	     ...
	     * */
	    deleteRelationOrPersonEventListener: function deleteRelationOrPersonEventListener() {
	        caches.deleteBtnEle.on('click', function () {
	            //获取选中节点
	            var index = d3.select(caches.currentSelectCircleEles[0]).attr('data-index');
